...
public class CreateDatasetSample {
    ...
    public static void main(String[] args) {
        ...
        // データセットを作成する
        Dataset dataset = dClient.createDataset(true, false, file);
        File image = new File("test.png");
        // データセットに画像を追加する
        DatasetAddImages images = dClient.addImagesToDataset(dataset.getId(), image);
        String imageId = images.getImages().get(0).getId();
        // 追加した画像をメイン画像に設定する
        dClient.setPrimaryImageToDataset(dataset.getId(), new SetPrimaryImageParam(imageId));
        ...
        dClient.changeAccessLevel(dataset.getId(), accesses);
    }
